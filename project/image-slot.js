...
(() => {
  ...
  // Cap the stored longest side. 3200px keeps photos crisp when a slot is
  // viewed large (fullscreen focus, wide screens, retina) while staying a
  // reasonable sidecar size. A 3200px WebP at q=0.95 is ~0.7-1.5MB.
  const MAX_DIM = 3200;
  ...
  // raw upload. Longest side is capped at 4× the slot's rendered width
  // (high-DPI + fullscreen headroom) and at MAX_DIM. Encoding is LOSSLESS:
  // WebP-lossless via OffscreenCanvas where supported (keeps alpha, ~half the
  // size of PNG), falling back to PNG (also lossless) otherwise.
  async function toDataUrl(file, targetW) {
    ...
    try {
      ...
      if (typeof OffscreenCanvas !== 'undefined') {
        ...
        const blob = await off.convertToBlob({ type: 'image/webp', lossless: true });
